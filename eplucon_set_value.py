...
async def eplucon_set_value(username = None, password = None, command = None, value = None, module_index = None, return_response=True):
    """yaml
...
"""
    ...
    command_type = {
        ...
        "boiler_temperature_delta": {"code":"5804", "type": "float", "min": "5.00", "max": "65.0"},
    ...
    if command_item["type"] == "float":
        if not isinstance(value, (float, int)) or (float(value) < float(command_item["min"]) or float(value) > float(command_item["max"])):
            return return_message("error", "The value " + str(value) + " is invalid for command, it expects a number with 1 decimal " + command)

    if command_item["type"] == "integer":
        if not isinstance(value, int) or (int(value) < int(command_item["min"]) or int(value) > int(command_item["max"])):
            return return_message("error", "The value " + str(value) + " is invalid for command, only whole numbers are expected for the command " + command)

    if command_item["type"] == "enum":
        if not isinstance(value, int) or (int(value) < int(command_item["min"]) or int(value) > int(command_item["max"])):
            return return_message("error", "The value " + str(value) + " is invalid for command " + command)

    if command_item["type"] == "boolean": 
        if not isinstance(value, int) or (int(value) < 0 or int(value) > 1):
            return return_message("error", "The value " + str(value) + " is invalid for command, a 0 or 1 is expected " + command)

    #setting the urls
    url_base = "https://portaal.eplucon.nl"
    url_authentication = url_base + '/login'
    url_getcontrol_postfix = '?blockType=module&account_module_index=' + module_index
    url_sendcontrol= url_base + '/e-control/ajax/send_control_data?account_module_index=' + module_index
    url_getcontrol = url_base + '/e-control/ajax/modal/' + command_item["code"] + url_getcontrol_postfix
    ...
    async with aiohttp.ClientSession() as session:
        ...
        form_data = {re.search(r'name="([^"]*)"', input_tag).group(0).replace('name=','').replace('"',''): re.search(r'value="([^"]*)"', input_tag).group(0).replace('value=', '').replace('"','') for input_tag in input_elements}
        ...
        input_elements = re.findall(r'<input[^>]*>', html)
        form_data = {re.search(r'name=\\"([^"]*)"', input_tag).group(0).replace('name=','').replace('"','').replace('\\',''): re.search(r'value=\\"([^"]*)"', input_tag).group(0).replace('value=','').replace('"','').replace('\\','') for input_tag in input_elements}
